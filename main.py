...
def get_median_execution_time_of_function(func, args, repeat=10):
    s = 0
    for i in range(10):
        s += timeit_and_exec(func, args)['execution_time']
    return s//repeat
...
def check_correctness_of_algorithms_of_algorithms():
    ...
    while True:
        target = random.choice(random.choice(matrix[12:]))+random.randint(0, 1)
